...
export const exercise: ExerciseService = {
  async generateSession({ dateISO, plan, capMin, focus = 'Strength', variant }) {
    const sessionId = `${dateISO}__v_${plan.version}__seq_01`;
    const buildStrength = (v?: string) => ([
      { type: 'warmup', items: ['bike 5min easy', 'hip openers'] },
      { type: 'main', exercises: [
        v === 'press' ? { id: 'bench_press', sets: 5, reps: 5, rir: 2 } : v === 'hinge' ? { id: 'deadlift', sets: 3, reps: 5, rir: 1 } : { id: 'back_squat', sets: 4, reps: 5, rir: 2 }
      ] },
      { type: 'accessory', exercises: [{ id: 'plank', sets: 3, timeSec: 60, rir: 2 }] },
      { type: 'cooldown', items: ['hamstring stretch', 'walk 5min'] }
    ]);
    const buildConditioning = (v?: string) => ([
      { type: 'warmup', items: ['row 5min easy'] },
      { type: 'main', exercises: [ v === 'steady' ? { id: 'jog_steady', sets: 1, timeSec: 1500, rir: 5 } : { id: 'airbike_intervals', sets: 10, timeSec: 30, rir: 3 } ] },
    ...
    const blocks = focus === 'Conditioning' ? buildConditioning(variant) : focus === 'Recovery' ? buildRecovery() : buildStrength(variant);
  ...
  async generateWeekAndValidate({ startISO, plan, recovery, capMin }) {
    ...
    let strengthDays = 0; let condDays = 0;
    for (let i = 0; i < 7; i++) {
      ...
      if (isBlocked || readiness <= 25) {
        ...
        if (baseFocus === 'Strength') { const order = ['squat','press','hinge']; variant = order[strengthDays % order.length]; strengthDays++; }
        else if (baseFocus === 'Conditioning') { variant = (condDays % 2 === 0) ? 'intervals' : 'steady'; condDays++; }
        const s = await this.generateSession({ dateISO: iso, plan, recovery, capMin: isDeload ? Math.round(cap * 0.8) : cap, focus: baseFocus, variant });
